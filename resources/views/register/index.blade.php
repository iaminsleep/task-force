        ...
        <section class="registration__user">
            ...
            <div class="registration-wrapper">
                <form class="registration__user-form form-create" method="post" action="{{ route('register.perform') }}">
                    ...
                    <input class="input textarea" id="16" name="email" placeholder="ivan@email.com" autocomplete="off" value="{{ old('email') }}"/>
                    <span>{{ $errors->first('email') }}</span>

                    <label @if($errors->has('name')) class="input-danger" @endif for="17">Ваше имя</label>
                    <input class="input textarea" id="17" name="name" placeholder="Иван Иванов" autocomplete="off" value="{{ old('name') }}"/>
                    ...
                    <select id="18" class="multiple-select input town-select registration-town" size="1" name="city_id">
                        @foreach($cities as $city)
                            <option value="{{ $city->id }}" @if((int) old('city_id') === $city->id) selected @endif>{{ $city->name }}</option>
                        @endforeach
                    </select>
                    <span>{{ $errors->first('city_id') }}</span>

                    <label @if($errors->has('password')) class="input-danger" @endif for="19">Пароль</label>
                    <input class="input textarea" type="password" id="19" name="password" autocomplete="off" value="{{ old('password') }}"/>
